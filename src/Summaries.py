...
# Data generation and plot generation are tracked separately so that
# a completed data pass does not suppress a subsequent plot pass.
# In-process sets dedupe within one Python interpreter. The filesystem
# markers below dedupe ACROSS multiprocessing workers (so 64 workers
# don't each re-run the simulation-wide aggregation).
_SIM_AGG_DATA_DONE  = set()    # tracks CSV data generation; tuples (simulationRoot, abnormal)
_SIM_AGG_PLOTS_DONE = set()    # tracks plot generation;     tuples (simulationRoot, abnormal)


def _simAggMarkerPaths(simulationRoot, abnormal, kind):
    base = os.path.join(simulationRoot, f".simagg_{kind}_{abnormal.lower()}")
    return f"{base}.lock", f"{base}.done"


def _claimSimAggWork(simulationRoot, abnormal, kind):
    """Cross-process guard. Returns True if THIS process should do the work.
    Returns False if another worker has already finished (or is currently doing) it.
    Atomic via os.mkdir; the .done marker is created after the work completes.
    """
    lock_path, done_path = _simAggMarkerPaths(simulationRoot, abnormal, kind)
    ...
    try:
        ...
        # Another worker is doing this (or did it and crashed before marking done).
        return False


def _markSimAggDone(simulationRoot, abnormal, kind):
    _, done_path = _simAggMarkerPaths(simulationRoot, abnormal, kind)
    try:
        with open(done_path, "w") as f:
            f.write("done\n")
    except Exception as e:
        logger.warning(f"Could not write sim-agg done marker {done_path}: {e}")
# ------------------------------------------------------------------
...
def calcProbabilitiesAllMCs(tss):
    # Combine all per-MC RLE segments into one big TimeseriesRLE and compute
    # the duration-weighted empirical PDF over emission rates. A prior branch
    # edit tried to route this through TimeseriesSet, but TimeseriesSet has no
    # toPDF — only TimeseriesRLE does. TimeseriesSet.sum/mean would compute a
    # per-timestamp aggregate, not the empirical sample distribution plotTs
    # expects to feed into hist().
    ts_df = pd.concat([t.df for t in tss.values()], ignore_index=True)
    combined_ts = ts.TimeseriesRLE(
        ts_df.sort_values(by=['nextTS'], ascending=[True]),
        filterZeros=False,
    )
...
def generatedCsvSummaries(config, df, site, abnormal):
    ...
    if simulationEmissions and key not in _SIM_AGG_DATA_DONE:
        if _claimSimAggWork(config['simulationRoot'], abnormal, 'data'):
            ...
            _markSimAggDone(config['simulationRoot'], abnormal, 'data')
        else:
            logger.info(f"Skipping sim-wide data aggregation for {key} — another worker owns it or it's done.")
        _SIM_AGG_DATA_DONE.add(key)
    ...
    if gen_plots:
        ...
        if os.path.exists(similationLevelSummariesPath):
            if key not in _SIM_AGG_PLOTS_DONE:
                if _claimSimAggWork(config['simulationRoot'], abnormal, 'plots'):
                    for sp in SPECIES:
                        plot_annual_emissions_unitid_level(
                    ...
                    _markSimAggDone(config['simulationRoot'], abnormal, 'plots')
                else:
                    logger.info(f"Skipping aggregated-simulation plots for {key} — another worker owns it or it's done.")
                _SIM_AGG_PLOTS_DONE.add(key)
            else:
